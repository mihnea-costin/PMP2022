...
# Incarcam datele in variabile separate
y = data["medv"]
...
with pm.Model() as model:
    ...
    # Parametrii pentru variabilele independente
    betas = pm.Normal("betas", mu=0, sd=10, shape=X.shape[1])
    # Variabila dependenta estimata
    y_est = alpha + pm.math.dot(X, betas)
    # Precizia (inversa variantei) erorilor
    tau = pm.Gamma("tau", alpha=0.1, beta=0.1)
    # Distributia normala a erorilor
